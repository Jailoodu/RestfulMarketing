...
In order to the run the tests inside the Docker container, run the command `docker ps` and copy the container ID. Then run the command `docker exec -it {container ID} sh`, replacing the `{container ID}` with the ID gathered from the previous command. Then run the command `python -m pytest --cov=api tests/`. A coverage report will be generated within `./docs/coverage.html` once the tests are completed.
